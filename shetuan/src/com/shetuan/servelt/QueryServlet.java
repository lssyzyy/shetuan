...
public class QueryServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (lru.equals("ulist")) {// 查询所有的member，结果集
		...
		}else if (lru.equals("slist")) {// 查询时所有的待审核社团，结果集

			CommunityDao s = new CommunityDao();
			String cpagestr = request.getParameter("cpage");
			System.out.println("communityServlet-获取所有待审核社团:" + cpagestr);
			int cpage = 1;
			if (cpagestr != null) {
				cpage = Integer.parseInt(cpagestr.trim());
			}
			Page page = new Page();
			page.setPageCurrent(cpage);
			List<Community> communities = s.getWaitCommunitiesForPage(page);
			request.setAttribute("waitcommunity", communities);
			request.setAttribute("page", page);
			// 跳转到社团列表界面
			request.getRequestDispatcher("waitcommunity_list.jsp").forward(request, response);
			
		}else if (lru.equals("jlist")) {// 查询时所有的待审核社团活动，结果集

			ActivityDao j = new ActivityDao();
			String cpagestr = request.getParameter("cpage");
			System.out.println("communityServlet-获取所有待审核活动:" + cpagestr);
			int cpage = 1;
			if (cpagestr != null) {
				cpage = Integer.parseInt(cpagestr.trim());
			}
			Page page = new Page();
			page.setPageCurrent(cpage);
			List<Activity> activities = j.getWaitActivitiesForPage(page);
			request.setAttribute("waitactivity", activities);
			request.setAttribute("page", page);
			// 跳转到社团活动列表界面
			request.getRequestDispatcher("waitactivity_list.jsp").forward(request, response);
