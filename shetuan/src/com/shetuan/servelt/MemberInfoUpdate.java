	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (isMultipart) {
				...
				while (iter.hasNext()) {
					...
					if (item.isFormField()) {
						...
						if (fileName.length() > 0) {
							...
							String sysDirPath = "D:\\新建文件夹 (3)\\shetuan\\WebContent\\header";
							// 
				...
				member.setSex(fromFilds.get("gender"));
				member.setAddress(fromFilds.get("add"));
				member.setGrade(fromFilds.get("grade"));
				member.setMemberInstitute(fromFilds.get("institute"));
				member.setEmail(fromFilds.get("email"));
				member.setPhonenumber(fromFilds.get("phone"));
				...
				System.out.println("status:" + fromFilds.get("status"));

				int result = memberdao.updateMember(member);
				String status=fromFilds.get("status");
				if(status.equals("index")) {
					Login login=new Login();
					login.setLoginName(member.getLoginName());
